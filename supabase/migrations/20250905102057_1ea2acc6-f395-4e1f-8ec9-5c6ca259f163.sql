...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' AND tablename = 'searches' AND policyname = 'Users can delete their own searches'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' AND tablename = 'search_results' AND policyname = 'Users can delete their own search results'
  ) THEN
